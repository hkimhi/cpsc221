...
{
    /* Your code goes here! */
    // make copies of both words
    std::string word1_copy = word1;
    std::string word2_copy = word2;

    // make the copies uppercase (in-place)
    ...
    // 
    auto lookup1 = dict.find(word1_copy);
    auto lookup2 = dict.find(word2_copy);

    if(lookup1 == dict.end() || lookup2 == dict.end()) {
        // either of the keys isn't found
        return false;
    }

    for(auto key_val1 : lookup1->second) {
        for(auto key_val2 : lookup2->second) {
            if(key_val1 == key_val2){
                // std::cout << word1_copy << " " << word2_copy << ", " << key_val1 << " " << key_val2 << std::endl;
                return true;
            }
        }
    }
