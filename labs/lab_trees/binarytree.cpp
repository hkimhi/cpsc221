 ...
 * @param subRoot The current node in the recusrion
 * @effect swaps the left and right branches of the Node
 */
template <typename T>
...
void BinaryTree<T>::printPaths(const Node* subRoot, vector<T> path) const
{
  ...
  {
    //subRoot is a leaf, so we print the path
    cout << "Path: ";
  ...
  {
    //subRoot is NOT a leaf, keep going down
    printPaths(subRoot->left, vector<T>(path));
    printPaths(subRoot->right, vector<T>(path));
  }
...
template <typename T>
int BinaryTree<T>::sumDistances(const Node* subRoot, int currHeight) const
{
  if(subRoot == NULL) return 0;

  return currHeight + sumDistances(subRoot->left, currHeight+1) + sumDistances(subRoot->right, currHeight+1);
}
